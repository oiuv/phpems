...
- 可在线[数据库迁移](https://learnku.com/docs/laravel/5.8/migrations/3928)，请参考 `tasks` 目录下 `Database.php` 和 `migrations` 目录下的示例文件
...
| [davedevelopment/phpmig](https://packagist.org/packages/davedevelopment/phpmig) | 数据库迁移指令 |
| [thamaraiselvam/mysql-import](https://packagist.org/packages/thamaraiselvam/mysql-import) | sql数据库导入组件 |
...
安装完成后，根据需要修改 lib 目录下的 `config.inc.php` 文件，配置数据库，然后使用 `migrations` 目录下的数据库文件 `phpems.sql` 创建数据库，**项目上线后务必删除 `tasks` 目录**。
