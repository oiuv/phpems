...
class action extends app
{
    ...
    private function makescore()
    {
        ...
        if ($this->ev->get('makescore')) {
            ...
            if ($id) {
                $message = ['statusCode' => 200, 'message' => '操作成功', 'callbackType' => 'forward', 'forwardUrl' => "index.php?exam-app-exam-makescore&ehid={$id}"];
            } else {
    ...
    private function score()
    {
        ...
        if ($this->ev->get('insertscore')) {
            ...
            if (!$needhand) {
                ...
                $this->exam->modifyExamSession($args);
                $message = ['statusCode' => 200, 'message' => '操作成功', 'callbackType' => 'forward', 'forwardUrl' => 'index.php?exam-app-exam-makescore&makescore=1&direct=1'];
            } else {
                if ($sessionvars['examsessionsetting']['examdecide']) {
                    ...
                    if ($id) {
                        $message = ['statusCode' => 200, 'message' => '操作成功，本试卷需要教师评分，请等待评分结果', 'callbackType' => 'forward', 'forwardUrl' => 'index.php?exam-app-history&ehtype=2'];
                    } else {
                        $message = ['statusCode' => 300, 'message' => '操作失败，请重新提交'];
                    }
                } else {
                    ...
                    $message = ['statusCode' => 200, 'message' => '操作成功', 'callbackType' => 'forward', 'forwardUrl' => 'index.php?exam-app-exam-score'];
                }
            }
            $this->G->R($message);
        } else {
    ...
    private function selectquestions()
    {
        ...
        if (!$r['examid']) {
            ...
            if (1 == $r['examtype']) {
                ...
                $message = ['statusCode' => 200, 'message' => '抽题完毕，转入试卷页面', 'callbackType' => 'forward', 'forwardUrl' => 'index.php?exam-app-exam-paper'];
                $this->G->R($message);
            } elseif (2 == $r['examtype']) {
                $sessionvars = $this->exam->getExamSessionBySessionid();
                $questions = [];
