	...
	<dl class="nowrap">
	  ...
	  <dd>
	  	...
	  	{x2;if:$type}
		<span class="info">&nbsp;分，已选<b id="ialreadyselectnumber_{x2;v:a2s0}">0</b>题（<a href="javascript:;" onclick="javascript:$.pdialog.open('?exam-master-exams-selected&questionids='+$('#iselectquestions_{x2;v:a2s0}').val()+'&rowsquestionids='+$('#iselectrowsquestions_{x2;v:a2s0}').val(),'selectedquestions','已选试题',{width:580});">查看</a>），描述&nbsp;</span><input type="text" name="args[examsetting][questype][{x2;v:a2s0}][describe]" value="" size="12"/><span class="info">&nbsp;【<a href="javascript:;" onclick="javascript:if($('#iselectallnumber_{x2;v:a2s0}').val() == '' || !parseInt($('#iselectallnumber_{x2;v:a2s0}').val()) || parseInt($('#iselectallnumber_{x2;v:a2s0}').val()) < 1)alert('请先输入总题数！');else $.pdialog.open('?exam-master-exams-selectquestions&search[questionsubjectid]={x2;$subjectid}&search[questiontype]={x2;v:a2s0}','selectquestions','选题',{max:true});">选题</a>】</span>
