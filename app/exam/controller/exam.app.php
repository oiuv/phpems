...
class action extends app
{
    ...
    private function ajax()
    {
        switch ($this->ev->url(4)) {
            ...
            case 'getexamlefttime':
                ...
                $sessionvars = $this->exam->getExamSessionBySessionid();
                if ($this->data['currentbasic']['basicexam']['opentime']['start'] && $this->data['currentbasic']['basicexam']['opentime']['end'])
                    $t = $this->data['currentbasic']['basicexam']['opentime']['end'] - 300; else
                    $t = TIME;
    ...
    private function view()
    {
        ...
        if ($sessionvars['examsessiontype'] != 2) {
            if ($sessionvars['examsessiontype'])
                header("location:index.php?exam-app-exampaper-view"); else
    ...
    private function makescore()
    {
        ...
        if ($this->ev->get('makescore')) {
            ...
            if ($this->ev->get('direct')) {
                if ($id)
                    header("location:index.php?exam-app-exam-makescore&ehid={$id}"); else
                    header("location:index.php?exam-app-exam-paper");
                exit;
            } else {
                if ($id)
                    $message = array('statusCode' => 200, "message" => "操作成功", "callbackType" => 'forward', "forwardUrl" => "index.php?exam-app-exam-makescore&ehid={$id}"); else
                    $message = array('statusCode' => 300, "message" => "操作失败，请重新提交");
                $this->G->R($message);
            }
        } else {
    ...
    private function score()
    {
        ...
        if ($this->data['currentbasic']['basicexam']['examnumber']) {
            ...
            if ($overflow) {
                $message = array('statusCode' => 300, "message" => "您的考试次数已经用完了！");
                $this->G->R($message);
            }
        }
        if ($this->ev->get('insertscore')) {
                    ...
                    foreach ($tmp as $p) {
                        if (is_array($sessionvars['examsessionuseranswer'][$p['questionid']])) {
                            ...
                            if ($nanswer == $p['questionanswer'])
                                $score = $sessionvars['examsessionsetting']['examsetting']['questype'][$key]['score']; else {
                            ...
                            if ($answer == $p['questionanswer'])
                                $score = $sessionvars['examsessionsetting']['examsetting']['questype'][$key]['score']; else $score = 0;
                        }
            ...
            if (!$needhand) {
                ...
                $this->exam->modifyExamSession($args);
                $message = array('statusCode' => 200, "message" => "操作成功", "callbackType" => 'forward', "forwardUrl" => "index.php?exam-app-exam-makescore&makescore=1&direct=1");
            } else {
                if ($sessionvars['examsessionsetting']['examdecide']) {
                    ...
                    if ($id)
                        $message = array('statusCode' => 200, "message" => "操作成功，本试卷需要教师评分，请等待评分结果", "callbackType" => 'forward', "forwardUrl" => "index.php?exam-app-history&ehtype=2"); else
                        $message = array('statusCode' => 300, "message" => "操作失败，请重新提交");
                } else {
                    $args['examsessionstatus'] = 1;
                    $this->exam->modifyExamSession($args);
                    //$this->favor->addExamHistory(1);
                    $message = array('statusCode' => 200, "message" => "操作成功", "callbackType" => 'forward', "forwardUrl" => "index.php?exam-app-exam-score");
                }
            }
            $this->G->R($message);
        } else {
    ...
    private function paper()
    {
        ...
        if ($sessionvars['examsessionstatus'] == 2) {
            ...
            if ($this->data['currentbasic']['basicexam']['selftemplate'])
                $this->tpl->display($this->data['currentbasic']['basicexam']['selftemplate']); else
    ...
    private function selectquestions()
    {
        ...
        if (!$r['examid']) {
            $message = array('statusCode' => 300, "message" => "参数错误，尝试退出后重新进入");
            $this->G->R($message);
        } else {
            if ($r['examtype'] == 1) {
                ...
                $message = array('statusCode' => 200, "message" => "抽题完毕，转入试卷页面", "callbackType" => 'forward', "forwardUrl" => "index.php?exam-app-exam-paper");
                $this->G->R($message);
            } elseif ($r['examtype'] == 2) {
                $sessionvars = $this->exam->getExamSessionBySessionid();
                $questions = array();
                ...
                if ($sessionvars['examsessionid'])
                    $this->exam->modifyExamSession($args); else
